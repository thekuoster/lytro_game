...
<div class="full-container">
    ...
    ?>
    <?php if(have_posts()) : ?>
        <div class="entry-content <?php priimo_portfolio_class(); ?>">
            <?php $count = 1; ?>
                    ...
                    <dl class='gallery-item portfolio-item' id="portfolio-item-<?php the_ID(); ?>">
                        <dt class='gallery-icon portfolio-icon' id="portfolio-icon-<?php the_ID(); ?>">
                            <a href="<?php the_permalink(); ?>">
                                <?php the_post_thumbnail('priimo-portfolio-image',array('title'	=> '')); ?>
                            </a>
                            <dd class="wp-caption-text gallery-caption" id="portfolio-title-<?php the_ID(); ?>"><?php the_title(); ?></dd>
                        </dt>
                    </dl>
                    <?php if($count % $priimo_options['portfolio_columns'] == 0) echo '<br style="clear: both" />'; $count++; ?>
                <?php endif; ?>
            <?php endwhile; ?>
            <?php wp_reset_query(); ?>
        </div>
